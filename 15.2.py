...
2.在第1步设置过cs试卷信息的可以跳过第2步。
  ...
  4）坐标采集方法在采集程序内。  
3.单击【选择文件夹】按钮，选择试卷所在文件夹。
4.单击【调整区域】按钮预览区域.第1个文本输入如10 10单击【调整区域】调整所有区域，输入-2 3 1只调整选项区域。
5.单击【开始阅卷】按钮，开始阅卷。
6. 试卷阅完后，第一个单行文本输入框内输入学生姓名，单击【查询学生】按钮进行查看。
7. 单击【导出excel】按钮统计分析学生成绩。
8.如果提示有卡没有识别，第一个文本输入框输入数字2，单击【开始阅卷】按钮查看情况。
9.白色太多，第二个文本输入框输入较小阈值，单击【阈值设置】按钮更改阈值，反之调大。
10.阈值输入框依次输入ej、xt、cj、ct、yj、bz,单击【阈值设置】按钮分别启动批改二卷、打小题分、采集试题库、
生成单个学生错题库、自动能所有学生发试题和错题本邮件、打开帮助等功能。
更多功能、操作演示请观看视频演示。
        """
        # 标签显示帮助
        ...
        if len(self.filelist) > 0:
            ...
            if str(self.filelist[self.filenumjsq])[-3:] != 'jpg':
                print('图片格式不是jpg!')
                # sys.exit(0)
        print('试卷人数为：{}'.format(len(self.filelist)))
        ...
        if self.pinjiekaiguan == 1:  # 左右拼接试卷
            ...
            for i in range(0, len(piclsit), 2):
                ...
                if Im0.size[0] > Im0.size[1]:  # 如果宽>高，剪切一部分拼接（理综）
                    ...
                    # if (self.pj3x-self.pj2x)>0 and (Im1.size[1]-self.pj1y*2-(self.pj3y-self.pj2y))>0:
                    #     Im3=Image.new('RGB', (self.pj3x-self.pj2x,Im1.size[1]-self.pj1y*2-(self.pj3y-self.pj2y)),(255,255,255))
                    #     # Im3.show()
                    #     logging.debug('产生空白图片正确')
                    #     result.paste(Im3, (self.pj1x, self.pj1y+self.pj3y-self.pj2y))
                    #     logging.debug('拼接空白图片正确')
    ...
    #     edged = cv2.Canny(autopic, 10, 100)  # 边缘检测
    ...
    #             if len(approx) == 4:
    #                 print(approx)
    #                 x = min(approx[0][0][0], approx[1][0][0], approx[2][0][0], approx[3][0][0]) - self.dw0x
    #                 y = min(approx[0][0][1], approx[1][0][1], approx[2][0][1], approx[3][0][1]) - self.dw0y
    #                 # print(68,79)
    #                 print(x, y)
    #
    #                 print(self.quyulist)
    ...
    def on_pushButton_daochuexcel_clicked(self):
        ...
        # 如果一卷前2列数据不为空，统计一卷成绩。
        if (self.sh1.cell(row=3, column=3).value is not None) or (self.sh1.cell(row=4, column=3).value is not None):
            # 对每一列即每一题进行统计
            for kk in range(self.tishu):
                ...
                self.sh2.cell(row=4, column=kk + 1).value = '正答率' + str(
                    int((len(listright) / len(listall)) * 100)) + '%'
                self.sh2.cell(row=5, column=kk + 1).value = '选A' + str(
                    int((strall.count('A')) / len(listall) * 100)) + '%'
                self.sh2.cell(row=6, column=kk + 1).value = '选B' + str(
                    int((strall.count('B')) / len(listall) * 100)) + '%'
                self.sh2.cell(row=7, column=kk + 1).value = '选C' + str(
                    int((strall.count('C')) / len(listall) * 100)) + '%'
                self.sh2.cell(row=8, column=kk + 1).value = '选D' + str(
        ...
        if 'erjuan' in self.chengjiexcel.sheetnames:
            ...
            if (erjuan.cell(row=3, column=3).value is not None) and (
                    erjuan.cell(row=4, column=3).value is not None):
                logging.debug("二卷已打开")
                rows = erjuan.max_row
                ...
                if cols > 1:
                    ...
                    # self.sh2.cell(row=18, column=1, value='II卷')
                    logging.debug('11')
                    ...
                    # self.sh2.cell(row=23, column=1, value='总分')
                    ...
                    for kk in range(3, self.sh1.max_row + 1):
                        if self.sh1.cell(row=kk, column=self.sh1.max_column).value != None:
                            # print(self.sh1.cell(row=kk + 3, column=2).value,
                            #       int(self.sh1.cell(row=kk + 3, column=3).value))
